<script lang="ts">
    ...
    export let l_server = server, r_server = server, l_usageConfig=usageConfig, r_usageConfig=usageConfig, l_formattedImpacts = formattedImpacts, r_formattedImpacts = formattedImpacts
    //Default values
    ...
    $: l_formattedImpacts=updateImpact(l_server);
    $: r_formattedImpacts=updateImpact(r_server);
...
<div id="content" class="px-1">
    ...
    <div class="grid md:grid-cols-12 gap-1">
        <div class="min-h-[200px] md:col-span-6 px-1 w-full ">
            <form> 
                ...
                <div id="serverconfig-usage" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 grid gap-1">
                    <ServerConfig bind:serverConfig={l_server} bind:usageConfig={l_usageConfig} {architems} {rammanufitems} {ssdmanufitems} />
                </div>
                <h2 class="m-2 text-2xl font-bold">{$_('server-config.usage')}</h2>
                <div id="serverconfig-usage" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 grid gap-1">
                    <UsageConfig bind:usage={l_server.usage} bind:usageConfig={l_usageConfig} usageType="server"/>
                    <p on:click={() => Utils.toggleElement("l_usageconfig-detailed")} class="ml-2 block w-full col-span-6"><a class="text-xs" href="javascript:void(0);" >> {$_('detailed-config.show-usage')}</a></p>
                    <div id="l_usageconfig-detailed" class="hidden col-span-6">
            ...
            <form> 
                <h2 class="mb-2 mx-2 text-2xl font-bold">{$_('server-config.configuration')}</h2>
                <div id="serverconfig-usage" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 grid gap-1">
                    <ServerConfig bind:serverConfig={r_server} bind:usageConfig={r_usageConfig} {architems} {rammanufitems} {ssdmanufitems} />
                </div>
                <h2 class="m-2 text-2xl font-bold">{$_('server-config.usage')}</h2>
                <div id="serverconfig-usage" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 grid gap-1">
                    <UsageConfig bind:usage={r_server.usage} bind:usageConfig={r_usageConfig} usageType="server"/>
                    <p on:click={() => Utils.toggleElement("r_usageconfig-detailed")} class="ml-2 block w-full col-span-6"><a class="text-xs" href="javascript:void(0);" >> {$_('detailed-config.show-usage')}</a></p>
                    <div id="r_usageconfig-detailed" class="hidden col-span-6">
                    <DetailedUsageConfig {serverImpact}/>
                    </div>
                </div>
            </form>
        ...
        <div class="grid md:grid-cols-12 gap-1">
                ...
                <div class="px-1 md:col-span-6">
            <h2 class="mb-2 mx-2 text-2xl font-bold">{$_('impacts.Results')}</h2>
                <ResultGrid verboseImpacts={l_formattedImpacts}/>
        </div>
        <div class="px-1 md:col-span-6">
            <h2 class="mb-2 mx-2 text-2xl font-bold">{$_('impacts.Results')}</h2>
                <ResultGrid verboseImpacts={r_formattedImpacts}/>
        </div>
    </div>
